...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        for i, idx in enumerate(indices):
            ...
            old_normal = self.normals[idx]
            old_color = self.colors[idx]
            old_weight = self.weights[idx]
            
            new_point = transformed_points[i]
            new_normal = transformed_normals[i]
            new_color = colors[i]
            
            self.points[idx] = (old_weight * old_point + new_point) / (old_weight + 1)
            self.colors[idx] = (old_weight * old_color + new_color) / (old_weight + 1)
            updated_normal = old_weight * old_normal + new_normal
            self.normals[idx] = updated_normal / np.linalg.norm(updated_normal)
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        dist_mask = dist < dist_diff
        
        normals /= np.linalg.norm(normals, axis=1, keepdims=True)
        input_normals /= np.linalg.norm(input_normals, axis=1, keepdims=True)
        dot_products = np.sum(normals * input_normals, axis=1)
        angle_mask = np.arccos(np.clip(dot_products, -1.0, 1.0)) < angle_diff
