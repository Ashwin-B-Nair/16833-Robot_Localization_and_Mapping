...
def find_projective_correspondence(source_points,
    ...
    T_source_normals = (R @ source_normals.T).T
    ...
    T_source_normals = T_source_normals[mask]
    
    # TODO: second filter: apply distance threshold
    proj_depth = T_source_points[:,2]
    vertex_depth = target_vertex_map[target_vs, target_us][:,2] 
    valid_vertex_mask = (vertex_depth >= 0) & (proj_depth >= 0)  & (~np.isnan(vertex_depth)) & (~np.isnan(proj_depth))
    dist_mask = valid_vertex_mask & (np.abs(proj_depth - vertex_depth) < dist_diff)
 
    n_q = target_normal_map[target_vs, target_us]
    dot_products = np.sum(T_source_normals * n_q, axis=1)
    T_source_normals = np.linalg.norm(T_source_normals, axis=1)
    n_q = np.linalg.norm(n_q, axis=1)
    cos_theta = dot_products / (n_q * T_source_normals)
    angle_mask = np.arccos(cos_theta) < np.deg2rad(5)
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    d_i = p_prime - q
    b = -np.sum(n_q*d_i, axis = 1)
    p_cross = np.cross(p_prime, n_q)
    A = np.concatenate((p_cross, n_q), axis=1)
    # End of TODO
...
def solve(A, b):
    ...
    Q, R = np.linalg.qr(A)   # Q is (N, 6) and R is (6, 6)
    Qt_b = Q.T @ b
    x = np.linalg.solve(R[:6,:], Qt_b[:6])         
    return x
...
if __name__ == '__main__':
    ...
    file_path = "C:/Users/ashwi/Documents/CMU/FALL_2024/SLAM/HW4/problem_set/problem_set/code/intrinsics.json"
    intrinsic_struct = o3d.io.read_pinhole_camera_intrinsic(file_path)
    ...
    # print("Target points shape:", target_vertex_map.reshape((-1,3)).shape)
