...
def odometry_estimation(x, i):
    ...
    pose_i = x[2*i:2*i+2]
    pose_i_plus_1 = x[2*(i+1):2*(i+1)+2]
    odom = pose_i_plus_1 - pose_i
    return odom
...
def compute_meas_obs_jacobian(x, i, j, n_poses):
    ...
    jacobian[0, 0] = delta[1] / d
    jacobian[0, 1] = -delta[0] / d
    jacobian[0, 2] = -delta[1] / d
    jacobian[0, 3] = delta[0] / d
    jacobian[1, 0] = -delta[0] / sqrt_d
    jacobian[1, 1] = -delta[1] / sqrt_d
    jacobian[1, 2] = delta[0] / sqrt_d
    jacobian[1, 3] = delta[1] / sqrt_d
...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom):
        ...
        # arr = np.hstack([np.eye(2), -np.eye(2)])
        arr = np.array([[-1, 0, 1, 0], [0, -1, 0, 1]])
        # A[2*i+2:2*i+4, pose_idx:pose_idx+4] = sqrt_inv_odom @ arr
        A[idx:idx+2, pose_idx:pose_idx+4] = sqrt_inv_odom @ arr
        odom_est = odometry_estimation(x, i)
        # b[2*i+2:2*i+4] = sqrt_inv_odom @ (odoms[i] - odom_est)
        b[idx:idx+2] = sqrt_inv_odom @ (odoms[i] - odom_est)
        idx += 2

    # TODO: Then fill in landmark measurements
    # meas_idx = 2 * (n_odom + 1)
    for k in range(n_obs):
        i = int(observations[k, 0])  # pose index
        j = int(observations[k, 1])  # landmark index
        pose_idx = 2*i
        landmark_idx = 2*n_poses + 2*j

        H = compute_meas_obs_jacobian(x, i, j, n_poses)
        A[idx:idx+2, pose_idx:pose_idx+2] = sqrt_inv_obs @ H[:, 0:2]
        A[idx:idx+2, landmark_idx:landmark_idx+2] = sqrt_inv_obs @ H[:, 2:4]

        obs_est = bearing_range_estimation(x, i, j, n_poses)
        err = observations[k, 2:] - obs_est
        err[0] = warp2pi(err[0])
        b[idx:idx+2] = sqrt_inv_obs @ err
